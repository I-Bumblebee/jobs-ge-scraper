...
class Pipeline:
    ...
    async def process_job_detail(
        self, job_id: str, job_url: str
    ) -> Tuple[str, JobDetailDict]:
        """Process a single job detail with proper error handling."""
        async with self._rate_limit():
            try:
                html = await self.collector.fetch_detail_page(
                    None, job_url
                )  # None as session will be managed inside
                job_detail, description = await self.parser.parse_job_detail(
                    html, job_id
                )
                ...
                job_description_path = await self.output_manager.save_description(
                    job_id, description
                )
                ...
                if job_detail_dict.get("dates") and isinstance(
                    job_detail_dict["dates"], dict
                ):
    ...
    async def process_job_stream(
        ...
        for i in range(0, len(all_job_ids), self.batch_size):
            ...
            self.logger.info(
                f"Processing batch {i // self.batch_size + 1}/{(len(all_job_ids) + self.batch_size - 1) // self.batch_size}"
            )
    ...
    async def run(self) -> None:
        ...
        async for job_id, data in self.process_job_stream():
            ...
            if isinstance(data, dict) and (
                "description_path" in data or "error" in data
            ):
        ...
        if remaining_jobs:
            self.logger.warning(
                f"{len(remaining_jobs)} jobs had incomplete data and weren't fully processed"
            )

        await self.output_manager.finalize()
        self.logger.info(
            f"Pipeline complete: {processed_count} jobs processed successfully, {error_count} errors"
        )
